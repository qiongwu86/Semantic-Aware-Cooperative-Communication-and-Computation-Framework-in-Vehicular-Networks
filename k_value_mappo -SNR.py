...
class ProbLinear(nn.Module):
    """概率化线性层，实现 Bayes-by-Backprop 式参数不确定性"""
    
    def __init__(self, in_features: int, out_features: int, bias: bool = True, 
                 sigma_init_min: float = 0.02, sigma_init_max: float = 0.10):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.use_bias = bias
        self.sigma_floor = 1e-6  # σ下界，防止塌缩
        
        # 权重参数：μ 和 ρ (σ = softplus(ρ))
        ...
        # 初始化参数
        self.sigma_init_min = sigma_init_min
        self.sigma_init_max = sigma_init_max
        self.reset_parameters()
        
    def reset_parameters(self):
        """初始化参数"""
        # μ 使用标准 He uniform 初始化
        ...
        # 当 ρ 较大时，σ ≈ ρ
        # 当 ρ 较小时，需要数值调整
        rho_min = math.log(math.expm1(self.sigma_init_min))  # 反向计算
        rho_max = math.log(math.expm1(self.sigma_init_max))
        nn.init.uniform_(self.weight_rho, rho_min, rho_max)
        
        if self.use_bias:
            nn.init.uniform_(self.bias_mu, -bound, bound)
            nn.init.uniform_(self.bias_rho, rho_min, rho_max)
    
    def get_weight(self):
        """获取当前采样的权重"""
        weight_sigma = F.softplus(self.weight_rho) + self.sigma_floor
        weight_eps = torch.randn_like(self.weight_mu)
        weight = self.weight_mu + weight_sigma * weight_eps
        
        if self.use_bias:
            bias_sigma = F.softplus(self.bias_rho) + self.sigma_floor
    ...
    def get_snr_loss(self, omega_max: float = 20.0):
        """计算 SNR 正则化损失"""
        # 权重 SNR
        weight_sigma = F.softplus(self.weight_rho) + self.sigma_floor
        weight_snr = torch.abs(self.weight_mu) / weight_sigma
        weight_loss = torch.mean(F.relu(weight_snr - omega_max) ** 2)
        ...
        if self.use_bias:
            bias_sigma = F.softplus(self.bias_rho) + self.sigma_floor
            bias_snr = torch.abs(self.bias_mu) / bias_sigma
            bias_loss = torch.mean(F.relu(bias_snr - omega_max) ** 2)
    ...
    def get_snr_stats(self):
        ...
        stats = {
            'weight_snr_mean': weight_snr.mean().item(),
            'weight_snr_max': weight_snr.max().item(),
        ...
        if self.use_bias:
            ...
            bias_snr = torch.abs(self.bias_mu) / bias_sigma
            stats.update({
                'bias_snr_mean': bias_snr.mean().item(),
                'bias_snr_max': bias_snr.max().item()
            })
...
class SharedActorCritic(nn.Module):
    """所有链路共享的Actor-Critic网络 - Actor使用参数不确定性，Critic保持确定性"""
    
    def __init__(self, obs_dim: int = 24, action_dim: int = 20, hidden_dim: int = 256,
                 enable_snr: bool = True, sigma_init_min: float = 0.02, sigma_init_max: float = 0.10):
        super().__init__()
        ...
        # Actor头：使用概率化参数
        if self.enable_snr:
            # 概率化 Actor：使用 ProbLinear 层
            self.actor_hidden = ProbLinear(hidden_dim, hidden_dim // 2, 
                                         sigma_init_min=sigma_init_min, sigma_init_max=sigma_init_max)
            self.actor_output = ProbLinear(hidden_dim // 2, action_dim,
                                         sigma_init_min=sigma_init_min, sigma_init_max=sigma_init_max)
        else:
            # 确定性 Actor（原始版本）
    ...
    def get_snr_loss(self, omega_max: float = 20.0):
        """获取 SNR 正则化损失"""
        if not self.enable_snr:
            return torch.tensor(0.0, device=next(self.parameters()).device)
        
        # 只对 Actor 的概率化层计算 SNR 损失
        snr_loss = self.actor_hidden.get_snr_loss(omega_max) + self.actor_output.get_snr_loss(omega_max)
        return snr_loss
    ...
    def get_snr_stats(self):
        ...
        # 收集所有 Actor 层的 SNR 统计
        hidden_stats = self.actor_hidden.get_snr_stats()
        output_stats = self.actor_output.get_snr_stats()
        
        # 重命名键以区分不同层
        stats = {}
        for key, value in hidden_stats.items():
            stats[f'actor_hidden_{key}'] = value
        for key, value in output_stats.items():
            stats[f'actor_output_{key}'] = value
            
        return stats
    
    def resample_actor_parameters(self):
        """重新采样 Actor 参数（每个环境步调用）"""
        if not self.enable_snr:
            return
        
        # 由于 ProbLinear 在每次前向传播时都重新采样，这里不需要额外操作
        # 但可以用于强制重新采样或其他操作
        pass
...
class KValueMAPPO:
    """K值选择的MAPPO训练器 - 所有链路共享同一策略网络（支持SNR参数不确定性）"""
    ...
    def __init__(self, 
                 ...
                 clip_coef: float = 0.2,
                 # SNR 模式下调整熵系数（参数随机性已提供探索）
                 ent_coef_init: float = 0.05,  # SNR模式下降低初始熵系数
                 ent_coef_final: float = 0.005,  # SNR模式下降低最终熵系数
                 ent_decay_steps: int = 100000,  # 熵衰减步数
                 vf_coef: float = 0.5,
                 max_grad_norm: float = 0.5,
                 update_frequency: int = 2048,  # 每2048个经验更新一次
                 # SNR 相关参数
                 enable_snr: bool = True,  # 是否启用SNR参数不确定性
                 snr_coef: float = 1e-4,  # SNR正则化系数
                 omega_max: float = 20.0,  # 最大SNR阈值
                 sigma_init_min: float = 0.02,  # 初始σ最小值
                 sigma_init_max: float = 0.10,  # 初始σ最大值
        ...
        # SNR 相关参数
        self.enable_snr = enable_snr
        self.snr_coef = snr_coef
        self.omega_max = omega_max
        
        # 网络（传递SNR参数）
        self.policy = SharedActorCritic(
            obs_dim, action_dim, hidden_dim, 
            enable_snr=enable_snr, 
            sigma_init_min=sigma_init_min, 
            sigma_init_max=sigma_init_max
        ).to(device)
    ...
    def update(self, batch_size: int = 64, update_epochs: int = 10):
        ...
        for epoch in range(update_epochs):
            ...
            for start in range(0, len(obs), batch_size):
                ...
                # SNR 正则化损失
                snr_loss = self.policy.get_snr_loss(self.omega_max) if self.enable_snr else torch.tensor(0.0, device=self.device)
        ...
        # 获取 SNR 统计信息
        snr_stats = self.policy.get_snr_stats() if self.enable_snr else {}
        ...
        result = {
            ...
            'snr_loss': total_snr_loss / update_epochs,
            'ent_coef': self.get_current_ent_coef(),
            'snr_coef': self.snr_coef,
            'training_steps': self.training_steps
        }
        
        # 添加 SNR 统计信息
        result.update(snr_stats)
...
class KValueTrainingEnvironment:
    ...
    def __init__(self, n_task_vehicles: int = 20, n_service_vehicles: int = 5, 
        ...
        # 合并用户配置
        if snr_config is not None:
            default_snr_config.update(snr_config)
        
        # MAPPO训练器（传递SNR配置）
        self.mappo = KValueMAPPO(**default_snr_config)
        ...
        # 训练历史记录（参照MADQN格式，增加SNR相关记录）
        self.loss_history = []
        self.policy_loss_history = []
        self.value_loss_history = []
        self.entropy_loss_history = []
        self.ent_coef_history = []
        self.snr_loss_history = []
        self.snr_stats_history = []
            ...
            for result in results:
                ...
                if self.mappo.should_update():
                    ...
                    if 'policy_loss' in losses:
                        ...
                        self.snr_loss_history.append(losses.get('snr_loss', 0.0))
                        
                        # 提取SNR统计信息
                        snr_stats = {k: v for k, v in losses.items() if k.startswith('actor_')}
                        self.snr_stats_history.append(snr_stats)
                        
                        # 计算总损失（与MADQN的loss_history对应，包括SNR损失）
                        total_loss = (losses['policy_loss'] + losses['value_loss'] + 
                                    losses['entropy_loss'] + losses.get('snr_loss', 0.0))
...
def main():
    ...
    # SNR 配置参数（可根据需要调整）
    snr_config = {
        'enable_snr': True,          # 是否启用SNR参数不确定性
        'snr_coef': 1e-4,           # SNR正则化系数（建议范围: 1e-5到5e-4）
                                    # 数值越大越保守，泛化性更好但收敛慢
        'omega_max': 20.0,          # 最大SNR阈值（建议范围: 10到30）
                                    # 数值越小约束越强，参数不确定性越高
        'sigma_init_min': 0.02,     # 初始σ最小值（控制参数噪声下界）
        'sigma_init_max': 0.10,     # 初始σ最大值（控制参数噪声上界）
        'ent_coef_init': 0.05,      # 降低的初始熵系数（SNR模式下可以更小）
        'ent_coef_final': 0.005     # 降低的最终熵系数（参数随机性已提供探索）
    }
    
    print(f"SNR 参数不确定性: {'启用' if snr_config['enable_snr'] else '禁用'}")
    if snr_config['enable_snr']:
        print(f"SNR 超参数: coef={snr_config['snr_coef']:.1e}, "
              f"omega_max={snr_config['omega_max']}, "
              f"sigma_range=[{snr_config['sigma_init_min']}, {snr_config['sigma_init_max']}]")
        ...
        if episode % 10 == 0:
            ...
            if 'policy_loss' in stats:
                print(f"           | "
                      ...
                      f"SNR Loss: {stats.get('snr_loss', 0.0):8.4f}")
                print(f"           | "
                      f"Ent Coef: {stats.get('ent_coef', 0.01):.4f} | "
                      f"SNR Coef: {stats.get('snr_coef', 0.0):.6f} | "
                      f"Actor SNR: {stats.get('actor_hidden_weight_snr_mean', 0.0):6.2f}")
        
        # 保存模型（修改保存路径到 mappo-snr 文件夹）
        if episode % 100 == 0 and episode > 0:
            import os
            os.makedirs('mappo-snr', exist_ok=True)
            model_path = f'mappo-snr/k_value_mappo_snr_episode_{episode}.pth'
            torch.save(train_env.mappo.policy.state_dict(), model_path)
            print(f"模型已保存: {model_path}")
    
    print("训练完成!")
    
    # 保存最终模型（到 mappo-snr 文件夹）
    import os
    os.makedirs('mappo-snr', exist_ok=True)
    final_model_path = 'mappo-snr/k_value_mappo_snr_final.pth'
    torch.save(train_env.mappo.policy.state_dict(), final_model_path)
    ...
    training_data = {
        ...
        'snr_loss_history': train_env.snr_loss_history,
        'snr_stats_history': train_env.snr_stats_history,
        'total_rewards': train_env.total_rewards,
        'total_performance_ratios': train_env.total_performance_ratios,
        # 记录SNR超参数
        'snr_config': {
            'enable_snr': train_env.mappo.enable_snr,
            'snr_coef': train_env.mappo.snr_coef,
            'omega_max': train_env.mappo.omega_max,
            'ent_coef_init': train_env.mappo.ent_coef_init,
            'ent_coef_final': train_env.mappo.ent_coef_final
        }
    }
    
    data_filename = f"mappo-snr/mappo_snr_training_data_{n_task_vehicles}vehicles.pkl"
